...
// Duration shrinks for longer moves: duration = DURATION * (1 - distance * DISTANCE_FADE),
// clamped to a minimum of DURATION_MIN. Distance is measured in a coordinate
// space where the screen height equals 2 units.
const float DISTANCE_FADE = 0.5;
const float DURATION_MIN = 0.15;

// Overall trail opacity (0 = invisible, 1 = fully opaque)
const float TRAIL_OPACITY = 0.15;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Duration scales inversely with distance traveled: long jumps resolve
    // quickly, while small movements linger. Distance is measured in a
    // coordinate space where the screen height equals 2 units.
    float distance = length(deltaPos);
    float duration = DURATION * clamp(1.0 - distance * DISTANCE_FADE, DURATION_MIN, 1.0);
    ...
    // Fade the trail out as the animation progresses
    alpha *= mix(1.0, 0.0, baseProgress) * TRAIL_OPACITY;
    ...
    // Keep the trail color close to the cursor color (no saturation boost)
    float gray = dot(iCurrentCursorColor.rgb, vec3(0.299, 0.587, 0.114));
    const float saturationBoost = 1.0;
